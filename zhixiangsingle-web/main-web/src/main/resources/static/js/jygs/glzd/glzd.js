...
var ajaxSCJCXXSearchFormOption = {
    ...
    url: "/managementSystem/getManagementSystemList",
    type: "post",
...
$(function(){
    ...
    loadCurrentPageStyle("经营公司","管理制度");
    ...
    $("#comInfoForm").validationEngine({
        ...
        ajaxFormValidationURL:"/managementSystem/setManagementSystem",
            ...
            if(data.result!=null){
                ...
                $.each(data.result,function (ind,val) {
                    if(val){
                        var comInfoTopStr = '';
                        var executionTime = '';
                        if(val.executionTime&&val.executionTime!=''){
                            executionTime = val.executionTime;
                        }
                        var systemName = '';
                        if(val.systemName&&val.systemName!=''){
                            systemName = val.systemName;
                        }
                        if(systemName!=''){
                            if(executionTime!=''){
                                comInfoTopStr = systemName+"-"+executionTime;
                            }else{
                                comInfoTopStr = systemName;
                            }
                        }else{
                            if(executionTime!=''){
                                comInfoTopStr = executionTime;
                            }
                        }
                        var contentDescribe = '';
                        if(val.contentDescribe&&val.contentDescribe!=''){
                            contentDescribe = val.contentDescribe;
                        }
                        var ibImgPath = getManyPicture(val.imgPath,val.sitePhoto,val.siteName,val.picturePrefix);
                        ...
                        $("#userListUl").append("<div class='col-md-3 col-sm-3 col-xs-12'>" +
                            ...
                            "<div class='tpl-i-font' style='height: 4.5em;' data-toggle2='tooltip' title='"+contentDescribe+"' data-original-title='"+contentDescribe+"'>" + contentDescribe +
                            "</div>" +
                            ...
                            "<button type='button' class='am-btn am-btn-default am-btn-danger' style='width: 50% !important;' onclick='delRole("+val.id+","+val.sdId+","+"\""+systemName+"\")'><i class='iconfont iconshanchutishi'></i> 删除</button>" +
                ...
                showJqueryConfirmWindow("#icon-tishi1","未获取到管理制度信息");
            }
            isShow = true;
        }else if(data.resultCode==101){
            showJqueryConfirmWindow("#icon-shangxin1","获取管理制度信息异常！深感抱歉，请联系管理人员为您加急处理");
        }else{
            showJqueryConfirmWindow("#icon-tishi1",data.msg);
        }
    }
...
function befShowAddMod(){
    ...
    //重置清空form
    myResetForm();
    $("#chgBusTitle").html("<i class='iconfont iconshicailiebiao2'></i>&nbsp;新增管理制度");
...
function updateRole(id,updSdId) {
    ...
    //isNaN是数字返回false
    if(id!=null && !isNaN(id)){
        $.post("/managementSystem/getUpdManagementSystem",{"id":id,"sdId":updSdId},function(data){
            if(!isKickOut(data)){
                //没有被踢出
                if(data.resultCode==100){
                    $("#id").val(data.result.id);
                    $("#sdId").val(data.result.sdId);
                    $("#imgPath").val(data.result.imgPath);
                    $("#systemName").val(data.result.systemName);
                    $("#contentDescribe").val(data.result.contentDescribe);
                    if(data.result.imgPath&&data.result.imgPath!=""){
                        var imgs = data.result.imgPath.split(",");
                    ...
                    $("#chgBusTitle").html("<i class='iconfont iconshicailiebiao2'></i>&nbsp;更新管理制度");
            ...
            buttons: {
                showMyMsg2: {
                    ...
                    action: function () {
                        ...
                        $.post("/managementSystem/delManagementSystem",{"id":id,"sdId":delSdId},function(data){
                            if(!isKickOut(data)){
                                //没有被踢出
                                if(data.resultCode==100){
                                    //刷新数据
                                    loadUserList();
                                    showJqueryConfirmWindow("#icon-jinlingyingcaitubiao14",'删除'+name+'制度成功！');
                                }else if(data.resultCode==101){
                                    showJqueryConfirmWindow("#icon-shangxin1","操作失败！深感抱歉，请联系管理人员为您加急处理");
                                }else{
                                    showJqueryConfirmWindow("#icon-tishi1",data.msg);
...
function exportSingle() {
    var exportSdIds = '';
    if($("#userSites").val()){
        exportSdIds = $("#userSites").val().toString();
    }
    window.location.href="/managementSystem/writeManagementSystems?sdIds="+exportSdIds+"&systemName="+$("#topSName").val();
}
