...
$(function(){
    ...
    loadCurrentPageStyle("经营公司","实景展示");
    loadDisplayArea();
    loadUserList();
    ...
    $("#comInfoForm").validationEngine({
        ...
        ajaxFormValidationURL:"/liveShow/setLiveShow",
...
function setErrorStyle() {
    setTimeout(function () {
        $("#areaId").prev().css("top","0px");
        $("#areaId").prev().css("left","15.9em");
    },1000);
}
    ...
    if(!isKickOut(data)){
        if(data.resultCode==100){
            if(data.result!=null){
                ...
                $.each(data.result,function (ind,val) {
                    if(val){
                        ...
                        if(val.area&&val.area!=''){
                            comInfoTopStr = val.area;
                        }
                        var ibImgPath = getManyPicture(val.img,val.sitePhoto,val.siteName,val.picturePrefix);
                        ...
                        $("#userListUl").append("<div class='col-md-3 col-sm-3 col-xs-12'>" +
                            ...
                            "<button type='button' class='am-btn am-btn-default am-btn-danger' style='width: 50% !important;' onclick='delRole("+val.id+","+val.sdId+","+"\""+comInfoTopStr+"\")'><i class='iconfont iconshanchutishi'></i> 删除</button>" +
                ...
                showJqueryConfirmWindow("#icon-tishi1","未获取到实景展示信息");
            }
            isShow = true;
        }else if(data.resultCode==101){
            showJqueryConfirmWindow("#icon-shangxin1","获取实景展示信息异常！深感抱歉，请联系管理人员为您加急处理");
        }else{
            showJqueryConfirmWindow("#icon-tishi1",data.msg);
        }
    }
...
function befShowAddMod(){
    ...
    $("#areaId").val("");
    $("#areaId").selectator('refresh');
    $("#areaId").parent().parent().css("display","block");
    /*$("#form-group-sdId").css("display","block");*/
    ...
    $("#chgBusTitle").html("<i class='iconfont iconshicailiebiao2'></i>&nbsp;新增实景展示");
...
function updateRole(id,updSdId) {
    ...
    $("#areaId").val("");
    $("#areaId").selectator('refresh');
    $("#areaId").parent().parent().css("display","none");
    $("#form-group-sdId").css("display","none");
    $.each($(".myImgFirst"),function(ind,val){
        $(val).parent().remove();
    });
    ...
    //isNaN是数字返回false
    if(id!=null && !isNaN(id)){
        $.post("/liveShow/getUpdLiveShow",{"id":id,"sdId":updSdId},function(data){
            if(!isKickOut(data)){
                //没有被踢出
                if(data.resultCode==100){
                    ...
                    if(data.result.img&&data.result.img!=""){
                        var imgs = data.result.img.split(",");
                    ...
                    $("#chgBusTitle").html("<i class='iconfont iconshicailiebiao2'></i>&nbsp;更新实景展示");
            ...
            buttons: {
                showMyMsg2: {
                    ...
                    action: function () {
                        ...
                        $.post("/liveShow/delLiveShow",{"id":id,"sdId":delSdId},function(data){
                            if(!isKickOut(data)){
                                //没有被踢出
                                if(data.resultCode==100){
                                    //刷新数据
                                    loadUserList();
                                    showJqueryConfirmWindow("#icon-jinlingyingcaitubiao14",'删除'+name+'实景成功！');
                                }else if(data.resultCode==101){
                                    showJqueryConfirmWindow("#icon-shangxin1","操作失败！深感抱歉，请联系管理人员为您加急处理");
                                }else{
                                    showJqueryConfirmWindow("#icon-tishi1",data.msg);
...
function loadDisplayArea() {
    $.post("/displayArea/getAll",function(data){
        if(!isKickOut(data)){
            //没有被踢出
            if(data.resultCode==100){
                displayAreaArray = data.result;
                $("#areaId").html("");
                $("#areaId").append("<option value=''>选择区域</option>");
                $.each(data.result,function (ind,val) {
                    $("#areaId").append("<option value='"+val.id+"' data-siteId='"+val.sdId+"' data-subtitle='"+val.siteName+"' data-left='<img src="+val.sitePhoto+">'>"+val.area+"</option>");
                });
                $("#areaId").selectator({
                    labels: {
                        search: '选择区域'
                ...
                $("#areaId").next().css("width","100%");
                $("#areaId").next().find(".selectator_input").css("background","#1b1b1b");
            }
        }
    });
}
...
function setMyCurId() {
    $("#sdId").val($("#areaId").find("option:selected").attr("data-siteId"));
}
