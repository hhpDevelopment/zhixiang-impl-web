...
var ajaxSCJCXXSearchFormOption = {
    ...
    url: "/displayArea/getDisplayAreaList",
    type: "post",
    data:{},
    dataType: "json",
    timeout: 12000
...
$(function(){
    ...
    loadCurrentPageStyle("经营公司","区域管理");
    ...
    $("#comInfoForm").validationEngine({
        ...
        ajaxFormValidationURL:"/displayArea/setDisplayArea",
                ...
                $.each(data.result,function (allInd,allRows) {
                    if(allRows&&allRows.rows!=undefined&&allRows.rows!=null&&allRows.rows.length>0){
                        var tab = '<table id="sort'+allInd+'" class="my-table">' +
                            ...
                            '</tr>'+
                            '<tr>' +
                            '<th><i class="iconfont iconzhanghuquanxian"></i>&nbsp;区域名称</th>' +
                        ...
                        $.each(allRows.rows,function(ind,val){
                            if (val.pid == 0) {
                                ...
                                $("#data"+allInd).append("<tr id='"+val.id+"-myStyleLi-"+val.sdId+"'>" +
                                    "<td data-label='区域名称' style='text-align: left;'>" +
                                    "<small class='label label-danger' style='font-size: 90%'>"+val.area+"</small>" +
                                    "</td>" +
                                    "<td data-label='操作'>" +
                                    ...
                                    "<i class='fa fa-trash-o' data-toggle='tooltip' title='删除' data-original-title='删除' style='cursor: pointer;font-size:1.3em;padding-left: 1em;' onclick='delPerm("+val.id+","+val.sdId+",\""+val.area+"\")'></i>" +
                                    "</div>" +
                                    "</td>" +
                                    "</tr>");
                                ...
                                if(sonArray.length>0){
                                    ...
                                    $.each(sonArray,function (ind2,val2) {
                                        $("#"+val.id+"-myStyleLi-"+val.sdId).after("<tr id='"+val2.id+"-myStyleLi-"+val2.sdId+"' class='box-body"+val.id+"-"+val.sdId+"'>" +
                                            "<td data-label='区域名称' style='text-align: left;padding-left: 3%;'>" +
                                            "<small class='label label-danger' style='font-size: 90%'>"+val2.area+"</small>" +
                                            "</td>" +
                                            "<td data-label='操作'>" +
                                            ...
                                            "<i class='fa fa-trash-o' data-toggle='tooltip' title='删除' data-original-title='删除' style='cursor: pointer;font-size:1.3em;padding-left: 1em;' onclick='delPerm("+val2.id+","+val2.sdId+",\""+val2.area+"\")'></i>" +
                                            "</div>" +
                                            "</td>" +
                                            "</tr>");
                                            ...
                                            $.each(sonArray2,function (ind3,val3) {
                                                if(val2.pid!=0){
                                                    $("#"+val2.id+"-myStyleLi-"+val2.sdId).after("<tr id='"+val3.id+"-myStyleLi-"+val3.sdId+"' class='"+gParent+" box-body"+val2.id+"-"+val2.sdId+"'>" +
                                                        "<td data-label='区域名称' style='text-align: left;padding-left: 5%;'>" +
                                                        "<small class='label label-danger' style='font-size: 90%'>"+val3.area+"</small>" +
                                                        "</td>" +
                                                        "<td data-label='操作'>" +
                                                        ...
                                                        "<i class='fa fa-trash-o' data-toggle='tooltip' title='删除' data-original-title='删除' style='cursor: pointer;font-size:1.3em;padding-left: 1em;' onclick='delPerm("+val3.id+","+val3.sdId+",\""+val3.area+"\")'></i>" +
                                                        "</div>" +
                                                        "</td>" +
                                                        "</tr>");
                                                }
...
function addSun(pid,flag,curSdId){
    ...
    if(null!=pid){
        ...
        if(flag==0){
            ...
            $("#pid").val(0);
            $("#chgBusTitle").html("<i class='iconfont iconshicailiebiao2'></i>&nbsp;新增区域");
        }else{
            //设置父id
            $("#pid").val(pid);
            $("#chgBusTitle").html("<i class='iconfont iconshicailiebiao2'></i>&nbsp;新增子区域");
        }
        //赋值类型
...
function editPerm(id,type,curSdId) {
    if(id!=null && !isNaN(id)){
        var subUrl = '/displayArea/getUpdDisplayArea';
        $("#form-group-sdId").css("display","none");
        //重置清空form
        myResetForm();
...
function delPerm(id,delSdId,name){
    if(null!=id&&id!=""){
        var subUrl = "/displayArea/delDisplayArea";
        $.confirm({
            icon: '#icon-tishi1',
            theme: 'modern',
            title: '提示',
            content: '您确定要'+name+'区域吗？',
            animation: 'news',//动画
                        ...
                        $.post(subUrl,{"id":id,"sdId":delSdId},function(data){
                            if(!isKickOut(data)){
                                ...
                                if(data.resultCode==100){
                                    //刷新数据
                                    loadUserList();
                                    showJqueryConfirmWindow("#icon-jinlingyingcaitubiao14",'删除'+name+'区域成功！');
                                }else if(data.resultCode==101){
                                    showJqueryConfirmWindow("#icon-shangxin1","操作失败！深感抱歉，请联系管理人员为您加急处理");
                                }else{
                                    showJqueryConfirmWindow("#icon-tishi1",data.msg);
